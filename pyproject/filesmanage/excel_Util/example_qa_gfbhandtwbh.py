...
for filename in os.listdir(source_folder):
    ...
    if filename.endswith('.json') or filename.endswith('.Json'):
        ...
        try:
            ...
            random_number = random.randint(1, element_count)
            iCountT = 0
            for entry in data:  
                iCountT = iCountT + 1
                if iCountT == random_number:
                    if "文档名称" in entry and "条文编号" in entry and "切片不带格式" in entry:
                        ...
                        if len(articlecode) < 9 and len(articlecode) > 4:
                            icount2 = icount2 + 1 
...
for index, item in enumerate(filenames):
    ...
    if match:
        ...
        print(result2)  # 输出：CJJ 83-2016
    else:
        result2 = "XXXXXXXX"

    result3 = result2 + " "+ "第" + result + "条"
