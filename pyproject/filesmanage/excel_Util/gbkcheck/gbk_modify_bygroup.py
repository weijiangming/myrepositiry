    ...
    if jsonname.endswith('.json') or jsonname.endswith('.Json'):
        ...
        try:
                ...
                if "文档名称" in entry and "条文编号" in entry and "切片不带格式" in entry and "切片带格式" in entry:
                    ...
                    if bsamefront:
                        if newsubstring in slicetexts:
                            ...
                            #情况1、2。
                            found, frontlist = isIndexInDictKey(indexT,frontindex_codes_dict,linkkey)
                            if found:#重复组已有在字典记录  
                                #情况1.index和front都在
                                if frontcode in frontlist:
                                    indexfront = frontcode + linkkey + str(indexT)
                                    try:
                                        frontindex_codes_dict[indexfront].append(articlecode)
                                    except Exception as e:
                                        print(f"An error occurred: {e}")
                                        print(len(frontindex_codes_dict))
                                        print(jsonname)

                                else:#情况2.index在和front不在
                                    foundtwbh = articlecodes[indexT]
                                    foundfront = get_string_before_last_dot(foundtwbh)
                                    if foundfront != frontcode:
                                        indexfront = frontcode + linkkey + str(sync_index)
                                        frontindex_codes_dict[indexfront] = [] 
                                    else:
                                        pass #这种情况待调试补上
                                        #frontindex_codes_dict[indexfront].append(articlecode)
                            else:
                                ...
                                if frontcode in frontlist:
                                    ...
                                    frontindex_codes_dict[indexfront] = []
                                    #frontindex_codes_dict[indexfront].append(articlecode)
                                    index2front_dict[indexT] = frontcode
                                else:#情况4 
                                    indexfront = frontcode + linkkey + str(indexT)
                                    frontindex_codes_dict[indexfront] = []
                                    frontindex_codes_dict[indexfront].append(articlecode)
            ...
            for entry in data:
                ...
                for key, value_list in frontindex_codes_dict.items():
                ...
                if found == False:#需要保留的都在new_data里
                    ...
                    indexfront = frontcode + linkkey + str(sync_index)
                    if indexfront in frontindex_codes_dict:
                        icount4 = icount4 + 1
                        #②对于去重后的分片处理：删除“切片不带格式”和“切片带格式”行首的条文编号，然后将所有重复分片各自的条文编号拿出来填被留下分片的条文编号字段中。并用“、”隔开
                        articlecoderep = articlecodes[sync_index]
                        newarticlecodes = articlecoderep
                        twbhlist = frontindex_codes_dict[indexfront]
